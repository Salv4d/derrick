...
- [ ] **CA2 — Finalize HybridProvider.** Land the pending docker+nix composed provider with a clear `Status()` that reports both legs independently, wire it through `ResolveProvider`, and document it.
- [ ] **CA3 — Route `derrick shell` through `Provider.Shell`.** Today `cmd/derrick/shell.go` hard-codes `engine.NewShellEngine().EnterSandbox(".derrick")`, which breaks docker-only and hybrid projects. Delegate to `provider.Shell(cfg)` so each backend owns the interactive-shell behavior.
- [ ] **CA4 — Remove dead code in `internal/engine/docker.go`.** `StartContainers` / `StopContainers` are orphaned since the Provider refactor. Delete to prevent drift and duplicated logic.
