...
- [x] **D2 — Document hybrid provider and multi-project behavior.** Cover when to use `provider: hybrid`, how `derrick shell` behaves, and what happens when multiple projects run concurrently (state lock, per-project docker network, port conflicts, shared /nix/store).
    - Added "Hybrid Provider" and "Multi-Project Behavior" sections to `website/docs/architecture.md`. Hybrid section includes a behavior matrix per operation (who runs, who no-ops, how errors aggregate) and a concrete "why use it" framing (polyglot services-in-containers + toolchain-in-nix). Multi-project section enumerates the load-bearing design decisions: state-file flock per `.derrick/`, per-project compose networks (no global `derrick-net`), port conflicts are user-visible by design, shared `/nix/store` is a feature not a risk, `DERRICK_START_CHAIN` cycle detection, and label-scoped `derrick clean`. While in the file I also corrected two stale items the audit surfaced: the `Provider.Shell` signature (missing `args []string` after CA3) and the "Project Clustering" section that still referenced the global `derrick-net` network removed back in v0.1.0.
